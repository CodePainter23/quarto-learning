...
![](results/horse_pops_plot.png){#fig-horsesall}

Horse populations for all provinces in Canada from 1906 - 1972 [@horses1].
:::

We can see from @fig-horsesall that Ontario, Saskatchewan and Alberta 
...
We present the standard deviations in @tbl-horsecount.


```{r}
#| label: tbl-horsecount
#| tbl-cap: "Standard deviation of historical (1906-1972) horse populations for each Canadian province [@horses2]."
#| message: false
#| warning: false
...
Additionally, note that in @tbl-horsecount we consider the sample standard deviation of the number of horses during the same time span as @fig-horsesall.

::: {#fig-horsesone}

![](results/horse_pop_plot_largest_sd.png){width=65%}

Horse populations for the province with the largest standard deviation.
:::

In @fig-horsesone we zoom in and look at the province of `{r} largest_sd`, which had the largest spread of values in terms of standard deviation.
